...
function SellModal({ holding, company, prices, portfolios, onClose, onDone }) {
  ...
  const handleSell = async () => {
    ...
    try {
      if (orderMode === "stoploss") {
        ...
        setOrderResponse({
          orderId: "PENDING", status: "STOP_LOSS_SET", orderType: "STOP_LOSS",
  ...
  if (step === "form") return (
    <Modal onClose={onClose}>
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: T.redDim, border: `1px solid ${T.redBdr}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 12, fontWeight: 800, color: T.red }}>{(company?.ticker || "?").slice(0, 2)}</span>
            </div>
            <div>
              <p style={{ margin: 0, fontWeight: 800, fontSize: 16, color: T.text }}>{company?.name}</p>
              <span style={{ fontSize: 10, fontWeight: 700, color: T.red, background: T.redDim, border: `1px solid ${T.redBdr}`, padding: "2px 7px", borderRadius: 5 }}>{company?.ticker}</span>
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ...
            <div key={label} style={{ flex: 1, background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 10, padding: "8px 12px" }}>
              ...
              <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Wallet Balance */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 10, padding: "10px 14px", marginBottom: 20 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => setQuantity((q) => Math.max(1, q - 1))}
            style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${T.border}`, background: T.surface2, color: T.textSub, fontSize: 20, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
          ...
          <button onClick={() => setQuantity((q) => Math.max(1, Math.min(maxQty, q + 1)))}
            style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${T.border}`, background: T.surface2, color: T.textSub, fontSize: 20, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>

          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <p style={{ margin: 0, fontSize: 11, color: T.textDim }}>You'll receive</p>
            <p style={{ margin: 0, fontSize: 17, fontWeight: 800, color: T.red, fontVariantNumeric: "tabular-nums" }}>₹{total}</p>
          </div>
        </div>
      </div>
...
const PortfolioPage = () => {
  ...
  const fetchOrders = useCallback(() => {
    if (!selected) return;
    axiosPortfolio.get("/orders/history", { params: { portfolioId: selected.id } })
  ...
  const fetchPendingOrders = useCallback(() => {
    if (!selected) return;
    axiosPending.get("/pending-orders", { params: { portfolioId: selected.id } })
