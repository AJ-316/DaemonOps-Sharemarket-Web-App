...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();  // ← this prevents refresh, must be here
    setError("");
    ...
    try {
      ...
      // backend returns LoginResponse { token: "..." }
      if (res.data?.token) {
        localStorage.setItem("token", res.data.token);
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  className="text-xs text-[#10B981] hover:text-[#059669] font-medium transition"
                >
                  Forgot password?
                </button>
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3 rounded-xl font-medium text-sm transition-all duration-200 mt-2 ${loading
                ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                : "bg-[#10B981] hover:bg-[#059669] text-white shadow-sm shadow-emerald-200 hover:shadow-md hover:shadow-emerald-200 active:scale-[0.98]"
                }`}
            >
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Don't have an account?{" "}
            <Link to="/register"
              className="text-[#10B981] hover:text-[#059669] font-medium transition">
              Create one
            </Link>
          </p>
        </div>
